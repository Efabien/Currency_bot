...
var Resource = NOOT.Object.extend(Authable).extend(Queryable).extend({
  ...
  /**
   * Cleaning `data` by `_id` and `select` fields
   *
   * @method _cleanupBySelection
   * @param {Object} data
   * @param {Object} select
   * @returns {Array}
   * @private
   */
  _cleanupBySelection: function(data, select) {
    var keys = _.chain(['_id']).concat(_.keys(select)).uniq().value();

    function getCleanItem(item) {
      var newItem = {};
      _.forEach(keys, function(key) { NOOT.trySet(newItem, key, NOOT.tryGet(item, key)); });
      return newItem;
    }

    if (_.isArray(data)) {
      return _.map(data, getCleanItem);
    }

    if (_.isObject(data)) {
      return getCleanItem(data);
    }
  ...
  formatResponsePackage: function(stack, callback) {
    ...
    // if there is a select qs, we override `minimize` settings
    if (NOOT.tryGet(stack, 'query.raw.select')) {
      pack.data = this._cleanupBySelection(pack.data, stack.query.select);
    }
  ...
  parseQueryFilter: function(stack, callback) {
    ...
    return async.each(Object.keys(filter), function(filterName, cb) {
      ...
      var field = _.find(fields, function(field) {
        if (field.isAny) return (new RegExp('^' + NOOT.toRegExpString(field.publicPath))).test(publicPath);
        return publicPath === field.publicPath;
      });
