...
var MongooseResource = MongoResource.extend({
  ...
  getMany: function(stack, callback) {
    ...
    callback = callback || stack.next;
    var isDelField = this.fields.isDel;

    if (isDelField && !query.allowDeleted) query.filter.isDel = { $ne: true };
    return this.model
      ...
      .exec(function(err, items) {
        if (err) return callback(NOOT.Errors.fromMongooseError(err));
        if (query.disableMeta) {
          stack.setData(items).setStatus(NOOT.HTTP.OK);
